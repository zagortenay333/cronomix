...
function markup_to_pango (string, markup_map) {
    ...
    // We loop over the string and look for delims as defined in @markup_map.
    ...
    for (let i = 0, string_l = string.length; i < string_l; i++) {
        ...
        if (delim === '\\') {
            ...
            backslash = false;
            continue;
        } else if (! markup_map.has(delim)) {
            continue;
        }

        // [delim, matching delim_tuple, pango_markup, delim.length, pos in text]
        ...
        i--; // we decrement because of while loop

        delim = delim_tuple[0];

        if (! markup_map.get(delim)) {
            continue;
        }
