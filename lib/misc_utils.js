...
// The function util_get_transformed_allocation() was deprecated
// in favor of get_transformed_extents() in gnome shell 3.38.
var get_transformed_allocation;
if (Config.PACKAGE_VERSION < '3.37') {
    get_transformed_allocation = Shell.util_get_transformed_allocation;
} else {
    get_transformed_allocation = (actor) => {
        let extents      = actor.get_transformed_extents();
        let top_left     = extents.get_top_left();
        let bottom_right = extents.get_bottom_right();
...
function open_file_path (path) {
    ...
    try {
        if (GLib.file_test(path, GLib.FileTest.EXISTS)) {
            Gio.app_info_launch_default_for_uri(GLib.filename_to_uri(path, null), global.create_app_launch_context(0, -1));
        } else {
...
function maybe_ignore_release (actor) {
    ...
    let a            = get_transformed_allocation(actor);
...
function scroll_to_item (scrollview, scrollbox, item, item_parent = scrollbox, horizontal = false) {
    ...
    // Do the scroll.
    {
...

        {
            ...
            let p1 = new Graphene.Point3D();
            [ p1.x, p1.y, p1.z ] = [ a.x1, a.y1, 0.0 ];

            let p2 = new Graphene.Point3D();
            [ p2.x, p2.y, p2.z ] = [ a.x2, a.y2, 0.0 ];

            p1 = item_parent.apply_relative_transform_to_point(scrollbox, p1);
            p2 = item_parent.apply_relative_transform_to_point(scrollbox, p2);
