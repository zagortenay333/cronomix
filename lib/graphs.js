...
var VBars = class VBars {
    ...
    draw_coord_system (params) {
        for (let prop in params) {
            if (!params.hasOwnProperty(prop) || !this.coord_info.hasOwnProperty(prop)) continue;
    ...
    draw_vbars (vbars, min_w, max_w, tooltip_format_callback) {
        ...
        if (tooltip_format_callback) this.tooltip_format_callback = tooltip_format_callback;
        this.vbars_drawing_area.queue_repaint();
    }

    // This func should never be called directly, only as a signal handler of the 'repaint' signal.
...
var HeatMap = class HeatMap {
    constructor () {
        ...
        // @tooltip_format_callback: func (square.label) -> string (used to format the tooltip label)
        this.params = {
            matrix               : [],
            matrix_size          : [],
            row_labels           : [],
    ...
    _draw_heatmap (area) {
        ...
        for (let row_label of this.params.row_labels) {
            let label = Pango.font_description_from_string(`bold ${this.params.row_label_size}px`);
