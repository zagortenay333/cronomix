...
//
// @lower_bound : string (in iso yyyy-mm-dd format. E.g., 2000-01-01)
// @upper_bound : string (in iso yyyy-mm-dd format. E.g., 2000-01-01)
...
var DatePicker = new Lang.Class({
    ...
    _init: function (lower_bound, upper_bound, labels, do_wrap) {
    ...
    get_date: function () {
        ...
        let date_str =
            '%d-%02d-%02d'.format(date[0], date[1] + 1, date[2]);

        return [date, date_str];
    },

    set_range: function (lower_bound, upper_bound) {
        this.lower_bound = lower_bound;
        this.upper_bound = upper_bound;
    },
    ...
    _on_date_changed: function (ymd, direction) {
        ...
        let new_date_str =
            '%d-%02d-%02d'.format(new_date[0], new_date[1] + 1, new_date[2]);

        let below = this.lower_bound && new_date_str < this.lower_bound;
        let above = this.upper_bound && new_date_str > this.upper_bound;

        if (below) {
            if (this.do_wrap && this.upper_bound)
                new_date_str = this.upper_bound;
            else
                new_date_str = this.lower_bound;
        }
        else if (above) {
        ...
        if (new_date_str !== prev_date_str) {
            this.emit('date-changed', new_date, new_date_str);
        }
