...
// Height of edges that run along the top/bottom of scrollview.
const UPPER_LOWER_EDGE_SCROLL = 100;
...
var Draggable = new Lang.Class({
    ...
    _on_drag_motion: function (drag_event) {
        ...
        if (this.scroll_view_alloc) {
            let upper_y = this.scroll_view_alloc.y1 + UPPER_LOWER_EDGE_SCROLL;
            let lower_y = this.scroll_view_alloc.y2 - UPPER_LOWER_EDGE_SCROLL;
            let [x, y,] = global.get_pointer();

            if (y < upper_y) {
                let delta = UPPER_LOWER_EDGE_SCROLL + 10 - upper_y + y;
                ...
                let delta = UPPER_LOWER_EDGE_SCROLL + 10 - y + lower_y;
                scroll_speed = delta < 10 ? scroll_speed : delta;
