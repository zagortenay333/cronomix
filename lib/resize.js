...
var MakeResizable = new Lang.Class({
    ...
    _update_mouse_cursor: function () {
        ...
        if (y > a.y1 && y < (a.y1 + w)) {
            if (x > a.x1 && x < (a.x1 + w)) {
                global.screen.set_cursor(Meta.Cursor.NW_RESIZE);
                this.resize_direction = ResizeDirection.NW;
            }
            else if (x > (a.x2 - w) && x < a.x2) {
                global.screen.set_cursor(Meta.Cursor.NE_RESIZE);
                this.resize_direction = ResizeDirection.NE;
            }
            else {
                global.screen.set_cursor(Meta.Cursor.NORTH_RESIZE);
                this.resize_direction = ResizeDirection.NORTH;
            }
        }
        else if (y > (a.y2 - w) && y < a.y2) {
            if (x > a.x1 && x < (a.x1 + w)) {
                global.screen.set_cursor(Meta.Cursor.SW_RESIZE);
                this.resize_direction = ResizeDirection.SW;
            }
            else if (x > (a.x2 - w) && x < a.x2) {
                global.screen.set_cursor(Meta.Cursor.SE_RESIZE);
                this.resize_direction = ResizeDirection.SE;
            }
            else {
                global.screen.set_cursor(Meta.Cursor.SOUTH_RESIZE);
                this.resize_direction = ResizeDirection.SOUTH;
            }
        }
        else if (x > a.x1 && x < (a.x1 + w)) {
            global.screen.set_cursor(Meta.Cursor.WEST_RESIZE);
            this.resize_direction = ResizeDirection.WEST;
        }
        else if (x > (a.x2 - w) && x < a.x2) {
            global.screen.set_cursor(Meta.Cursor.EAST_RESIZE);
