...
var MakeResizable = class MakeResizable {
    ...
    _maybe_begin_resize () {
        ...
        this.resizing = true;
        this.start_box = MISC.get_transformed_allocation(this.actor);
    ...
    _update_mouse_cursor () {
        ...
        let a = MISC.get_transformed_allocation(this.actor);
        ...
        if (y > a.y1 && y < (a.y1 + w)) {
            if (x > a.x1 && x < (a.x1 + w)) {
                ...
                global.display.set_cursor(Meta.Cursor.NORTH_RESIZE);
            ...
            if (x > a.x1 && x < (a.x1 + w)) {
                ...
                global.display.set_cursor(Meta.Cursor.SOUTH_RESIZE);
                this.resize_direction = ResizeDirection.SOUTH;
            }
        }
        else if (x > a.x1 && x < (a.x1 + w)) {
            global.display.set_cursor(Meta.Cursor.WEST_RESIZE);
            this.resize_direction = ResizeDirection.WEST;
        }
        else if (x > (a.x2 - w) && x < a.x2) {
            global.display.set_cursor(Meta.Cursor.EAST_RESIZE);
