...
export type Events = {
    focus_enter:   true;
    focus_leave:   boolean; // returns has_pointer
    pointer_enter: true;
    pointer_leave: boolean; // returns has_focus
}

export class FocusTracker extends PubSub<Events> {
    has_focus   = false;
    has_pointer = false;
    ...
    constructor (widget: St.Widget) {
        ...
        this.#widget.connect('enter-event', () => { if (!destroyed && this.#check_pointer()) this.publish('pointer_enter', true); })
        this.#widget.connect('leave-event', () => { if (!destroyed && !this.#check_pointer()) this.publish('pointer_leave', this.has_focus) })
        this.#widget.connect('notify::mapped', () => {
            if (destroyed) {
                // nothing
            } if (this.#widget.is_mapped()) {
                if (! this.#sid) this.#sid = global.stage.connect('notify::key-focus', () => this.#check_focus());
            } else {
                this.disconnect();
            }
        });
    ...
    #check_pointer (): boolean {
        if (! this.#widget.is_mapped()) return false;
