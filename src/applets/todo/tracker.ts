...
export type TrackerSlot = {
    ...
    // but rather a fresh task object representing the
    // entry in the time tracker file.
    task: Task;

    // The keys of this object are date strings in the
    // ISO format year-month-day.
    time: Record<string, T.Seconds>;
}
...
export class TrackerStats {
    ...
    custom = {
        ...
        until: '',
        total: 0 as Seconds,
        slots: Array<{ slot:TrackerSlot, total:Seconds }>(),
    };
}
...
export class TimeTracker extends PubSub<TrackerEvents> {
    file  = '';
    slots = Array<TrackerSlot>();
    tic   = 0; // 0 when paused
    time  = new T.Time(0); // 0 when paused
    tracked_slot: TrackerSlot|null = null;
    ...
    set_file (file: string): string {
        ...
        for (const [idx, slot] of this.slots.entries()) {
            ...
            (slot.task as any).toJSON = this.#task_to_json;
    ...
    get_slot (task: Task): TrackerSlot | null {
        ...
        if (! slot) {
            ...
            (slot_task as any).toJSON = this.#task_to_json;
...
export class TimeTrackerView {
    ...
    #query_until_entry: Entry;
    #query_info_button: Button;
    #query_sort: Dropdown;
    #query_markup: Markup;
    #custom_date_range_label = '';
    #per_task_query_title: St.Label;
    #query_tasks_scroll: LazyScrollBox;

    constructor (applet: TodoApplet, task_to_query?: Task) {
        this.#applet = applet;
        ...
        sdc_copy_button.subscribe('left_click', () => {
            ...
            result.custom.slots = result.custom.slots.map(x => ({ slot: x.slot.task.text, total: x.total })) as any;
        ...
        this.#query_sort.on_change = sort => {
            applet.storage.modify('tracker_query', x => x.value.sort = sort as TrackerSort);
            this.#update_query_column();
        }
        let counter = 0;
        this.#tracked_slot_id = tracker.subscribe('tic', () => {
            ctc_time_label.text = tracker.time.fmt_hms();
            if (counter++ === 5) { counter = 0; this.#update_ui(); }
        });
    ...
    #update_query_column () {
        ...
        { // Compute stats:
            ...
            for (const slot of tracker.slots) {
                if (! slot.task.satisfies_filter(filter)) continue;
                ...
                for (const [day, time] of Object.entries(slot.time)) {
                    ...
                    for (const [key, date] of Object.entries(dates)) {
                        if (day >= date && day <= dates.today) {
                            this.#stats[key as keyof T.SpecialDates] += time;
                        }
                    }
                }

                if (slot_custom_range_total) this.#stats.custom.slots.push({ slot, total: slot_custom_range_total });
            }
        }
        ...
        { // Render total times table:
            ...
            for (const [key, tr] of Object.entries(T.SpecialDatesTr)) {
                const time = this.#stats[key as keyof T.SpecialDates];
    ...
    * #gen_task_stats (): IterableIterator<St.Widget> {
