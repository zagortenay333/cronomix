...
export class TimerApplet extends Applet<Events> {
    ...
    set_preset (preset_idx: number) {
        const presets = this.storage.read.presets.value;
        if (preset_idx >= presets.length) preset_idx = -1;
    ...
    delete_preset (preset_idx: number) {
        this.storage.modify('presets', x => Misc.array_remove_idx(x.value, preset_idx));
        if (this.storage.read.current_preset.value === preset_idx) this.set_preset(-1);
    }
...
class MainView {
    ...
    constructor (applet: TimerApplet) {
        this.#applet = applet;
...
class PresetCard extends Misc.Card {
    constructor (applet: TimerApplet, preset: Preset, preset_idx: number) {
        super();
...
class PresetEditor extends EditorView {
    constructor (applet: TimerApplet, preset?: Preset) {
        super();
        ...
        ok_button.subscribe('left_click', () => {
            ...
            if (time.total === 0) {
                ...
                if (current_idx === preset_idx && applet.state === State.RESET) applet.reset(preset_idx);
            } else {
